...
@click.option(
    "--json-only",
    is_flag=True,
    help="Write JSON reports only (skip Markdown).",
)
@click.option(
    "--md-only",
    is_flag=True,
    help="Write Markdown report only (skip JSON).",
)
def main(
    input_dir: Path,
    output_dir: Path,
    ...
    """Analyze Word and PDF documents; output JSON + migration readiness summary."""
    ...
    console.print("[dim]No API key required - automated analysis engine[/dim]")
    ...
    if not md_only:
        ...
        console.print(f"[green]OK[/green] JSON master report -> {paths['master']}")
        console.print(f"[green]OK[/green] Per-document JSON -> {output_dir / 'documents'} ({len(paths) - 1} files)")
    ...
    console.print("\n[bold green]Done.[/bold green] Open analysis_report.json for full structured output.")
